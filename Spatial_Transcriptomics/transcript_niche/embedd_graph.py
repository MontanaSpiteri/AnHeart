...
outnpy =  sys.argv[2]
outnpy_meta =  sys.argv[3]

trained_model =  sys.argv[4]
gpickle = sys.argv[5]
gpickle_meta = sys.argv[6]

number_of_samples1 =  int(sys.argv[7])
number_of_samples2 =  int(sys.argv[8])
min_comp = int(sys.argv[9])
...
with open(gpickle, 'rb') as f:
    g = pickle.load(f)
## graph meta saved before. In filter_and_subgraph.py
...
logger.info("graph meta last index " + str(graph_meta.index[-1]))
...
node_ids = g.nodes()
node_gen = GraphSAGENodeGenerator(g, batch_size, num_samples).flow(node_ids, g.nodes())

logger.info("checking generator set-up...")
first_batch = next(iter(node_gen))
print("Batch Content:", first_batch)
print("Shapes and types:", [(x.shape, type(x)) for x in first_batch[0]])
