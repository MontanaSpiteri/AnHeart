...
#buildFullGraph_input_dmax
# 
#conda environment: /graphsage
# coding: utf-8
import tensorflow
import tensorrt
import numpy as np
...
logging.basicConfig(filename=sys.argv[8], 
...
raw_csv = sys.argv[2]
xenium_gene_panel = sys.argv[3]
fullgraph_meta_csv = sys.argv[4]
components_len_csv = sys.argv[5]
dmax = float(sys.argv[6])
out_gpickle = sys.argv[7]
...
barcodes_df = pd.read_csv(samplefile, compression='gzip', sep = ",", header=0)
logger.info("Filtering low quality transcripts QV < 20 and only keeping nuclei transcripts")
barcodes_df = barcodes_df[barcodes_df['qv'] >= 20]
barcodes_df = barcodes_df[barcodes_df['nucleus_distance'] == 0]
...
# xenium_gene_panel
tagList_df = pd.DataFrame(np.unique(xenium_gene_panel['gene'].values), columns=['gene'])
