...
# Neuron cell type annotation

Re-cluster and annotate Neuron cell cluster

```{r}
sce_neuron <- sce_comb[, sce_comb$annotation == "Neuron"]

# re-cluster neuron population
set.seed(0010101010)
sce_neuron <- runUMAP(sce_neuron, dimred="X_scVI",
                    BPPARAM=MulticoreParam(10), n_neighbors = 15, spread = 3, min_dist = 0.1)

scater::plotReducedDim(sce_neuron, dimred = "UMAP", color_by = "annotation")

set.seed(0010101010)
Clusters <- scran::clusterCells(sce_neuron, use.dimred="X_scVI", 
                                BLUSPARAM=SNNGraphParam(k=55, type="rank", cluster.fun="leiden"))
table(Clusters)

sce_neuron$Cluster <- Clusters

df_neuron <- as.data.frame(reducedDim(sce_neuron, "UMAP"))
df_neuron$Cluster <- sce_neuron$Cluster
df_neuron$sample <- sce_neuron$sample
df_neuron$Sample_Type <- sce_neuron$Sample_Type
df_neuron$Patient <- sce_neuron$Patient

colours_clusters <- Polychrome::palette36.colors(17)
names(colours_clusters) <- 1:17

ggplot(df[sample(1:nrow(df_neuron)),], aes(x=UMAP1, y=UMAP2, color=Cluster)) + geom_point(alpha=0.5) + 
  theme_classic() +
guides(colour = guide_legend(override.aes = list(alpha = 1, size=3))) + scale_color_manual(values=colours_clusters) 

## NPC-like
marker_genes <- c("SOX11", "HES6", "STMN2", "LMO4", "TUBB3", "RAB3C", "S100A1", "DLX5", "ELAVL4", "SOX4", "CCND2", "RBM6", "SET", "TRA2A")
lapply(marker_genes, function(marker) markers(sce_neuron, marker))

## Excitatory Neuron
marker_genes <- c("NRN1", "HOPX", "HS3ST4", "EPHA4", "LMO4")
lapply(marker_genes, function(marker) markers(sce_neuron, marker))
...
# assign neuron annotations
annotations <- read.csv(file = "/data/process/annotations_neuron_subtypes.csv")
sce_neuron$annotation <- annotations$annotation[match(sce_neuron$Cluster, annotations$cluster)]
sce_neuron$annotation <- factor(sce_neuron$annotation)
df_neuron$annotation <- sce_neuron$annotation

ggplot(df[sample(1:nrow(df_neuron)),], aes(x=UMAP1, y=UMAP2, color=annotation)) + geom_point(alpha=0.5) + 
  theme_classic() + 
guides(colour = guide_legend(override.aes = list(alpha = 1, size=3))) + scale_color_manual(values=colours_comb) 
```

```{r}
# Plot neuron sub-types on original UMAP
sce_orig <- sce_comb
sce_orig$annotation <- factor(
    sce_orig$annotation,
    levels = unique(c(levels(sce_orig$annotation), levels(factor(sce_neuron$annotation))))
)
valid_indices <- match(colnames(sce_orig), colnames(sce_neuron))
sce_orig$annotation <- as.character(sce_orig$annotation)
sce_neuron$annotation <- as.character(sce_neuron$annotation)
sce_orig$annotation[valid_indices] <- sce_neuron$annotation[valid_indices]
sce_orig$annotation <- factor(sce_orig$annotation)

plotReducedDim(object = sce_orig, dimred = "UMAP", colour_by = "annotation") + theme(aspect.ratio = 1)+ ggtitle("Neurons")+ scale_color_manual(values=colours_comb)

df_neuron <- as.data.frame(reducedDim(sce_orig, "UMAP"))
df_neuron$annotation <- sce_orig$annotation

samp1 <- sample(1:nrow(df_neuron))

plot <- ggplot(df_neuron[samp1,], aes(x=UMAP1[samp1], y=UMAP2[samp1])) + 
      geom_point(col="black", size=0.9) +
      geom_point(col="white", size=0.5) +
      geom_point(aes(col=annotation[samp1]), size=0.455, alpha=0.75) +
      scale_color_manual(values=colours_comb) + themes +
...
```{r}
# "Synaptic Signaling" Neuron sub-type boxplot
gene_sets <- readRDS("/public_datasets/gene_sets/all_genesets_list.rds")
gene_sets <- gene_sets[grep("GOBP_SYNAPTIC_SIGNALING", names(gene_sets))]
gene_sets <- gene_sets[[1]]
genes <- gene_sets[gene_sets %in% rownames(sce_neuron)]

combined_data <- cbind(as.data.frame(colData(sce_neuron)), as.data.frame(t(logcounts(sce_neuron[genes, ]))))
combined_data <- combined_data %>%
  dplyr::mutate(
    cell_avg_expr = rowMeans(across(all_of(genes)), na.rm = TRUE),  # Per-cell average for the gene set
    cell_expressing = rowMeans(across(all_of(genes)) > 0, na.rm = TRUE)  # Per-cell proportion of expressed genes
  )

summary_data <- combined_data %>%
  dplyr::group_by(Patient, annotation, Sample_Type) %>%
  dplyr::summarize(
    avg_expr = mean(cell_avg_expr, na.rm = TRUE),  # Average of per-cell averages
    prop_expressing = mean(cell_expressing, na.rm = TRUE),  # Proportion synaptic module genes expressed by cells
    .groups = "drop"
  )
summary_data$Sample_Type <- factor(summary_data$Sample_Type, levels = c("Post-treatment", "Pre-treatment"))

summary_data <- summary_data %>%
  dplyr::mutate(annotation_Sample_Type = paste(annotation, Sample_Type, sep = "_"))

unique_categories <- unique(summary_data$annotation_Sample_Type)
alternating_colors <- rep(c("#0C7BDC", "#E66100"), length.out = length(unique_categories))
names(alternating_colors) <- unique_categories

plot <- ggplot(summary_data, aes(x = annotation_Sample_Type, y = avg_expr, fill = annotation_Sample_Type)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  
  geom_point(aes(color = Patient), position = position_jitter(width = 0.2), size = 2) +
  scale_fill_manual(values = alternating_colors) + 
  scale_color_manual(values = colours) +
  theme_classic() +
  labs(
    x = NULL,
    y = "Average log expression (Synaptic Signaling)",
    color = "Patient"
  ) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
...
# density
df_sub <- df_prog %>% filter(Sample_Type == "Post-treatment")
...
semantic$Pathway <- factor(semantic$Pathway, levels = c("other", "DNA damage and stress", "lipid / energy metabolism", "Chromosome and DNA related", "cell structure", "secretion and cell communication", "neural synapse related", "inflammation", "aerobic glycolysis/ energymetabolism", "Cell division and ribosome"))
...
desired_order <- c("other", "DNA damage and stress", "lipid / energy metabolism", "Chromosome and DNA related", "cell structure", "secretion and cell communication", "neural synapse related", "inflammation", "aerobic glycolysis/ energymetabolism", "Cell division and ribosome")
...
plot <- ggplot(expr_data_long, aes(x = Treatment, y = mean, colour = Patient)) +
  geom_boxplot(aes(group = Treatment, fill = Treatment), alpha = 0.9, outlier.shape = NA) + 
...
pathway_interest <- c("type1_IFN_pathway" = "GO:0060337", "IFN_gamma_mediated_pathway" = "GO:0060333", "TNF_pathway" = "GO:0033209")
go_pathways <- lapply(pathway_interest, function(x)
      AnnotationDbi::select(org.Hs.eg.db, keytype="GOALL", 
        keys=x, columns="SYMBOL"))
go_pathways <- lapply(go_pathways, function(x) unique(x$SYMBOL))
names(go_pathways) <- names(pathway_interest)

# check overlapping genes
overlap <- intersect(intersect(go_pathways$type1_IFN_pathway, pathways.glioma$`AC-like`), pathways.glioma$Astrocyte) # no overlap
overlap <- intersect(go_pathways$IFN_gamma_mediated_pathway, pathways.glioma$`AC-like`) # no overlap
overlap <- intersect(pathways.glioma$`AC-like`, pathways.glioma$Astrocyte) # no overlap

counts(sce_tum) <- logcounts(sce_tum)
go_pathways <- lapply(go_pathways, function(x) intersect(x, rownames(sce_tum)))

aggregated <- sumCountsAcrossFeatures(sce_tum, go_pathways,
    exprs_values="logcounts", average=TRUE, BPPARAM=MulticoreParam(5))

df <- cbind(as.data.frame(colData(sce_tum)),
  t(as.matrix(logcounts(sce_tum[genes_interest,]))))
rownames(aggregated) <- gsub(" ", "_", rownames(aggregated))
df <- cbind(df, t(aggregated))
```

```{r}
# Correlation plots per tumor cell type

# AC-like vs IFN
aclike <- as.character(pathways.glioma$`AC-like`)
aclike <- intersect(aclike, rownames(sce_tum))
aclike_expr <- colMeans(logcounts(sce_tum[aclike, ]))
df$AClike <- aclike_expr[colnames(sce_tum)]

df_summary <- df %>%
  group_by(Patient, Sample_Type, annotation) %>%
  summarize(
    AC_like_avg = mean(AClike, na.rm = TRUE),
    IFN_avg = mean(type1_IFN_pathway, na.rm = TRUE)
  ) %>%
  pivot_wider(
    names_from = Sample_Type,
    values_from = c(AC_like_avg, IFN_avg)
  ) %>%
  mutate(
    Change_AC_like = AC_like_avg_Post-treatment - AC_like_avg_Pre-treatment,
    Change_IFN = IFN_avg_Post-treatment - IFN_avg_Pre-treatment
  )

correlation_data <- df_summary %>%
  group_by(annotation) %>%
  summarize(
    cor_test = list(cor.test(Change_AC_like, Change_IFN, method = "pearson")), 
    .groups = "drop"
  ) %>%
  mutate(
...
unique_annotations <- unique(df_summary$annotation)
plots <- list()
for (anno in unique_annotations) {
  anno_data <- df_summary %>% filter(annotation == anno)
  anno_correlation <- correlation_data %>% filter(annotation == anno)
  
  p <- ggplot(anno_data, aes(x = Change_AC_like, y = Change_IFN)) +
    geom_smooth(method = "lm", color = "black", se = FALSE) + 
    geom_point(aes(color = Patient), size = 5) +
    geom_text(
      data = anno_correlation,
    ...
    labs(
      title = paste(anno),
      x = "Change in AC-like Program (Post- vs Pre-treatment)",
      y = "Change in IFN Program (Post- vs Pre-treatment)",
      color = "Patient"
    ) +
    theme_classic() +
    scale_color_manual(values = colours)
...
# HIF1 alpha (stress response) vs IFN (run plot as above)
hif1_alpha_genes <- c(
  "HIF1A", "ARNT", "EGLN1", "EGLN2", "EGLN3", "VHL", "FIH1",
  "VEGFA", "ANGPT2", "PDGFB", "SLC2A1", "HK1", "HK2", "PFKFB3",
  "LDHA", "PGK1", "ENO1", "EPO", "BNIP3", "BCL2", "TFRC", 
  "SLC11A2", "MMP2", "MMP9", "HMOX1", "CDKN1A", "NOS2", "ADM"
)
hif1_alpha_genes <- intersect(hif1_alpha_genes, rownames(sce_tum))
aclike_expr <- colMeans(logcounts(sce_tum[hif1_alpha_genes, ]))
df$AClike <- aclike_expr[colnames(sce_tum)]

# STING pathway vs IFN (run plot as above)
sting_pathway_genes <- c(
  "TMEM173", "CGAS", "TBK1", "IRF3", "IFI16", "DDX41",
  "NFKB1", "RELA", "STAT1", "STAT2", "IKBKE", "SLC19A1", "ZBP1", "AIM2"
)
sting_pathway_genes <- intersect(sting_pathway_genes, rownames(sce_tum))
aclike_expr <- colMeans(logcounts(sce_tum[sting_pathway_genes, ]))
df$AClike <- aclike_expr[colnames(sce_tum)]

# Change in immune proportion vs IFN
all_diffs <- all_diffs[all_diffs$annotation == "Immune", ] # all_diffs vector from differential proportion calculations above (all cell types)
all_diffs$Difference <- all_diffs$Difference * -1 # reverse direction so Post-treatment vs Pre-treatment
all_diffs$annotation <- NULL

df_summary <- df_summary %>%
  dplyr::left_join(all_diffs %>% dplyr::select(Patient, Difference), by = "Patient")

correlation_data <- df_summary %>%
  group_by(annotation) %>%
  summarize(
    cor_test = list(cor.test(Difference, Change_IFN, method = "pearson")), 
    .groups = "drop"
  ) %>%
  mutate(
    correlation = sapply(cor_test, function(x) x$estimate), 
...
for (anno in unique_annotations) {
  ...
  p <- ggplot(anno_data, aes(x = Difference, y = Change_IFN)) +
    geom_smooth(method = "lm", color = "black", se = FALSE) + 
    geom_point(aes(color = Patient), size = 5) +
    geom_text(
      data = anno_correlation,
    ...
    labs(
      title = paste(anno),
      x = "Change in Immune Proportion (Post- vs Pre-treatment)",
      y = "Change in IFN Program (Post- vs Pre-treatment)",
      color = "Patient"
    ) +
    theme_classic() +
...
```{r}
# AC-like vs OPC-like program correlation plot - per tumor cell type
opclike <- as.character(pathways.glioma$`OPC-like`)
opclike <- intersect(opclike, rownames(sce_tum))
opclike_expr <- colMeans(logcounts(sce_tum[opclike, ]))
df$OPClike <- opclike_expr[colnames(sce_tum)]

df_summary <- df %>%
  group_by(Patient, Sample_Type, annotation) %>%
  summarize(
    AC_like_avg = mean(AClike, na.rm = TRUE),
    OPC_like_avg = mean(OPClike, na.rm = TRUE)
  ) %>%
  pivot_wider(
    names_from = Sample_Type,
    values_from = c(AC_like_avg, OPC_like_avg)
  ) %>%
  mutate(
    Change_AC_like = AC_like_avg_Post-treatment - AC_like_avg_Pre-treatment,
    Change_OPC_like = OPC_like_avg_Post-treatment - OPC_like_avg_Pre-treatment
  )

correlation_data <- df_summary %>%
  group_by(annotation) %>%
  summarize(
    cor_test = list(cor.test(Change_AC_like, Change_OPC_like, method = "pearson")), 
    .groups = "drop"
  ) %>%
  mutate(
    correlation = sapply(cor_test, function(x) x$estimate), 
...
for (anno in unique_annotations) {
  ...
  p <- ggplot(anno_data, aes(x = Change_AC_like, y = Change_OPC_like)) +
    geom_smooth(method = "lm", color = "black", se = FALSE) + 
    geom_point(aes(color = Patient), size = 5) +
    geom_text(
      data = anno_correlation,
    ...
    labs(
      title = paste(anno),
      x = "Change in AC-like Program (Post- vs Pre-treatment)",
      y = "Change in OPC-like Program (Post- vs Pre-treatment)",
      color = "Patient"
    ) +
    theme_classic() +
    scale_color_manual(values = colours)
...
glycolysis_genes <- c("HK1", "HK2", "HK3", "GPI", "PFKL", "PFKM", "PFKP", 
                      ...
                      "ENO2", "ENO3", "PKM", "PKLR")
glycolysis_genes <- glycolysis_genes[glycolysis_genes %in% rownames(sce_comb)]

krebs_genes <- c("CS", "IDH1", "IDH2", "IDH3A", "IDH3B", "IDH3G", "OGDH", "OGDHL", "SDHA", "SDHB", "SDHC", "SDHD", "FH", "MDH1", "MDH2")
krebs_genes <- krebs_genes[krebs_genes %in% rownames(sce_comb)]

interest <- gene_sets[grep("GOBP_LACTATE_METABOLIC_PROCESS", names(gene_sets))]
lactate_genes <- interest[[1]]
lactate_genes <- lactate_genes[lactate_genes %in% rownames(sce_comb)]

sce_prog <- sce_comb[, sce_comb$annotation == "Progenitor"]
sce_prog$Info <- paste0(sce_prog$Patient, "_", sce_prog$Sample_Type)

sce_prog$glycolysis_score <- colMeans(logcounts(sce_prog[glycolysis_genes, ]))
sce_prog$krebs_score <- colMeans(logcounts(sce_prog[krebs_genes, ]))
sce_prog$lactate_score <- colMeans(logcounts(sce_prog[lactate_genes, ]))

plot_data <- data.frame(
  Patient = sce_prog$Patient,
  CellType = sce_prog$annotation,
  Treatment = sce_prog$Sample_Type,
  Info = sce_prog$Info,
  Glycolysis = sce_prog$glycolysis_score,
  Krebs = sce_prog$krebs_score,
  Lactate = sce_prog$lactate_score
)
...
# Per patient
for (patient in unique(plot_data$Patient)) {
  patient_data <- subset(plot_data, Patient == patient)
  ...
  plot <- ggtern(patient_data, aes(x = Glycolysis, y = Krebs, z = Lactate, fill = Treatment)) +
